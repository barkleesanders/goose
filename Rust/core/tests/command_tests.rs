...
fn command_definitions_cover_generated_protocol_command_map_ids() {
    // The protocol command map is an external codegen artifact committed under
    // <repo>/docs/generated/. It is not produced by this crate; on checkouts
    // without it this test self-skips instead of failing the whole suite.
    // (The path was previously `../../../../` which resolved ABOVE the repo
    // root — a bug; corrected to a CARGO_MANIFEST_DIR-relative path.)
    let map_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../docs/generated/protocol-command-map.md"
    );
        ...
        Err(_) => {
            eprintln!(
                "skipping command_definitions_cover_generated_protocol_command_map_ids: \
                 {map_path} not present (external codegen artifact)"
            );
            return;
        }
    };
...
fn official_app_emulator_fixture_promotes_validated_shortcut_commands() {
    // This fixture is captured from the official WHOOP app talking to a macOS
    // BLE emulator (provenance capture_kind=official_app_to_macos_emulator). It
    // is the author's private capture corpus and is not produced by any in-repo
    // generator, so on checkouts without it this test self-skips instead of
...
fn command_capture_plan_summarizes_emulator_evidence_promotion_work() {
    // Author's private official-app emulator capture (see
    // official_app_emulator_fixture_promotes_validated_shortcut_commands). Not
    // generated by any in-repo tool; self-skip when absent.
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../fixtures/command-evidence/whoop-emulator-command-evidence.json");
    if !path.exists() {
        eprintln!(
