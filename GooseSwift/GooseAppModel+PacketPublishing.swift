...
extension GooseAppModel {
  ...
  // Forwards to the internally-synchronized UI-state aggregator, so it is safe
  // to publish status from the off-main ingest/parse queues.
  nonisolated func publishPipelinePerformanceStatus(_ status: String) {
    packetUIStateAggregator.set(.performancePipelineStatus, status)
  }

  // Called from the off-main parse queue. The throttling timestamp is guarded
  // by pipelinePerformanceLogLock and it only touches the Sendable BLE log and
  // UI-state aggregator.
  nonisolated func recordRustBridgeTiming(
